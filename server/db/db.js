...
//请自行更改用户名和密码
mongoose.connection.openUri("mongodb://sergey:chang@localhost:27017/blog");
	...
	db.user.find({},(err,doc) => {
		if(err){
			...
			if(!doc.length){
				...
				new db.user({user: "7Ethan",password: md5("zhang123"+salt),salt: salt,lastLogin: currTime}).save()
			}else{
