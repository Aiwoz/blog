<template>
    <div class = "article-root-box">
    	<h3 v-if = "!tags.length"  class = "none-article">There are no articles to display...</h3>
    	<div class = "article-root" v-if = "tags.length">
    		<div class = "articles-sum">Total：{{ articles.sum }} posts</div>
	    	<div class = "stage-list" v-show = "!rotate">
	  			...
	  			<ul class = "rotate" @click = "leftSlider" ref = "container">
					<li v-for = "(item,index) in tags" ref = "degItem">
						<div class = "deg-item-mask">
							<a href = "javascript: void(0)" @click.stop = "jumpArticle(item.tag)">
								{{ index+1 }}.{{ item.tag }} （{{ item.num }} posts）
							</a>
						</div>
					</li>
				</ul>
	  		...
	  		<div class = "change-stage" v-show = "hiddenChange">
	    		<button v-show = "show3D" @click = "changeStage('rotate')" :class = "{'active-bg': rotate}">3D mode</button>
	    		<button @click = "changeStage('list')" :class = "{'active-bg': !rotate}">List mode</button>
	    	</div>
	    </div>	
    </div>
</template>
<script>
	...
	export default {
		...
		metaInfo(){
			return {
				title: "Overview -7Ethan",
				meta: [{vmid: "description",name: "description",content: "Overview -7Ethan"}]
			}
		},
		mounted(){
			this.regBrowser()
