				...
				<div class = "login-box" @click.stop  v-show = "maskShow">
					...
					<div class = "self-login">
						<div class = "design-name">
							...
							<input type = "text" placeholder = "your nickname" v-model = "designName" @focus = "userInfoErr.name = '' ">
						</div>
						<div class = "name-info">{{userInfoErr.name}}</div>
						<div class = "email">
							<label for = "">Email：</label>
							<input type = "text" placeholder = "your email" v-model = "designEmail"  @focus = "userInfoErr.email = '' ">
						</div>
					...
					<div class = "third-login">
						<div class = "line">Login with</div>
						<a href = "javascript: void(0)" onclick = "return window.open('https://graph.qq.com/oauth2.0/authorize?client_id=1106777072&response_type=token&scope=all&redirect_uri=https://www.7ethan.top/qc_back.html', 'oauth2Login_10000' ,'height=525,width=585, toolbar=no, menubar=no, scrollbars=no, status=no, location=yes, resizable=yes');" class = "login-qq">
...
<script>
	...
	export default {
		...
		mounted(){
			if(!localStorage.getItem("Ethan_blog_userInfo")){
				this.qq_user_info()
			}else{
				this.getLocal()
			}
		...
		methods: {
			...
			register: function(){
				...
				if(!this.userInfoErr.name && !this.userInfoErr.email){
					this.searchUser({name: this.designName}).then((data) => {
						if(data.exist === "yes"){
							that.userInfoErr.name = "Login has been token，try another one ？"
						}else{
							that.saveUser({
			...
			validateReg: function(){
				...
				if(dName.length === 0){
					this.userInfoErr.name = "Please input your name."
				}else if(pattern.test(dName)){
					this.userInfoErr.name = "Login has been token，try another one ？"
				}else if(dName.length > 8){
					this.userInfoErr.name = "Sorry,that's 8 character limit."
				}else if(nReg.test(dName)){
					this.userInfoErr.name = "Username cannot be all numeric."
				}else{
					this.userInfoErr.name = ""
				}
				if(dEmail.length === 0){
					this.userInfoErr.email = "Please input your email."
				}else if(!eReg.test(dEmail)){
					this.userInfoErr.email = "Correct email is needed."
				}else{
			...
			getLocal: function(){
				if(localStorage.getItem("Ethan_blog_userInfo")){
					let info = JSON.parse(localStorage.getItem("Ethan_blog_userInfo"))
					this.set_user({
						name: info.name,
						imgUrl: info.imgUrl,
						email: info.email	
			...
			setLocal: function(){
				...
				localStorage.setItem("Ethan_blog_userInfo",s_info)
			}
		}
	}
</script>
