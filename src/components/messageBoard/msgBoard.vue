	...
	<div class = "msgboard">
		...
		<div class = "say-box">
			<div v-show = "replyInfo.aite.length">
				<strong>Reply：@</strong><span>{{ replyInfo.aite }}</span>
				<span @click = "replyInfo.aite = ''" class = "exit-aite" :title = "'Cancel reply' + replyInfo.aite" >x</span>
			</div>
			<textarea  v-model = "sayWords" @focus = "showLogin" placeholder = "Please edit your message  😄 "></textarea>
			<div class = "icon-submit-box">
				<div class = "icon-userInfo-box">
					...
					<div class = "reviewer-info" v-show = "!!userInfo.name">
						...
						<a href = "javascript: void(0)" @click = "loginOut" >Exit</a>
					</div>
				</div>
				<input ref = "pubButton" type = "button" value = "post" @click = "postLeaveW"/>
			</div>
		</div>
							...
							<li v-for = "rep in item.reply" class = "board-reply-item">
								...
								<div class = "board-content">
									...
									<div class = "board-content-details">
										...
										<a href = "#anchor-msgBoard" class = "board-details-reply"><span  @click = "reply(item._id,rep.name)"> replied </span></a>
									</div>
								</div>
							</li>
						</ul>
			...
			<div class = "mask" v-show = "dialogErr.show"  @click = "dialogErr.show = false">
				<transition name = "dialog">
					<div class = "dialog" @click.stop>
						...
						<div><button @click = "dialogErr.show = false">ok</button></div>		
					</div>
				</transition>
			</div>		
		</transition>
...
<script>
	...
	export default {
		...
		metaInfo(){
			return {
				title: "Message -7Ethan",
				meta: [{vmid: "description",name: "description",content: "Message -7Ethan"}]
			}
		},
		...
		methods: {
			...
			postLeaveW: function(){
				...
				let content = this.productContent(),
					that = this,
					ui = this.userInfo
				this.$refs.pubButton.value = "sending..."
				if(this.replyInfo.firstLevel){
					this.saveLeaveWords({
						...
						if(data._id){
							setTimeout(() => {
								that.$refs.pubButton.value = "message"
								that.sayWords = ""
			...
			validatePub: function(){
				...
				if(!this.sayWords.length){
					this.dialogErr = {show: true,info: "Message can not be empty"}
					return true
				}
				if(this.sayWords.length > 300){
					this.dialogErr = {show: true,info: "The message is too long, please do not exceed 300 characters"}
					return true
				}
			},
