	...
	<div class = "comment">
		...
		<div class = "say-box">
			<div v-show = "aite.length">
				<strong>Reply：@</strong><span>{{ aite }}</span>
				<span @click = "aite = ''" class = "exit-aite" :title = "'cancel reply' + aite" >x</span>
			</div>
			<textarea  v-model = "sayWords" @focus = "showLogin" placeholder = "Please edit your comment 😄 "></textarea>
			<div class = "icon-submit-box">
				...
				<input ref = "pubButton" type = "button" value = "Post" @click = "publishComment"/>
			</div>
		</div>
				...
				<li v-for = "item in comments" class = "reviewer-item">
					<div class = "reviewer">
						...
						<div class="rev-details">
							...
							<a href = "#anchor-comment" class = "rev-details-reply"><span @click = "rep(item._id,item.name)"> Reply </span></a>
						...
						<ul>
							<li v-for = "reply in item.reply">
								...
								<div class = "ans-details">
									...
									<a href = "#anchor-comment" class = "ans-details-reply"><span  @click = "rep(item._id,reply.name)" > Reply </span></a>
			...
			<div class = "mask" v-show = "dialogErr.show"  @click = "dialogErr.show = false">
				<transition name = "dialog">
					<div class = "dialog" @click.stop>
						...
						<div><button @click = "dialogErr.show = false">Sure</button></div>
					</div>
				</transition>
			</div>
		</transition>
	...
	export default {
		...
		methods: {
			...
			publishComment: function(index){
				...
				//直接回复文章，一级评论
				if(!this.replyOthers){
					this.$refs.pubButton.value = "Working..."
					this.postComment({
						...
						if(data._id){
							setTimeout(() => {
								that.$refs.pubButton.value = "Post your comment"
								that.sayWords = ""
			...
			validatePub: function(){
				...
				if(!this.sayWords.length){
					this.dialogErr = {show: true,info: "Content cannot be empty"}
					return true
				}
				if(this.sayWords.length > 500){
					this.dialogErr = {show: true,info: "The content is too long, please do not exceed 500 characters"}
					return true
				}
			},
